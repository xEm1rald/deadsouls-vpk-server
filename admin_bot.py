...
async def cmd_start(message: Message):
    ...
    help_text = (
        ...
        "👤 `/user [id]` - Инфо о пользователе (введи ID из БД, TG ID или HWID)\n"
        "💳 `/pay [id]` - Платежи пользователя\n"
        "🎁 `/gifts` - Список кодов\n"
        "➕ `/give [id] [дни]` - Выдать подписку вручную\n"
        "💻 `/sethwid [id] [hwid]` - Сменить HWID (напиши None, чтобы отвязать)\n"
...
async def cmd_gifts(message: Message):
    ...
    # Запрашиваем 0-ю (первую) страницу при вводе команды
...
async def cb_gifts_page(callback: CallbackQuery):
    ...
    # Достаем номер страницы из callback_data (например, из "giftpage_1" достаем 1)
    page = int(callback.data.split("_")[1])

    text, kb = await get_gifts_page_content(page)

    try:
        # Меняем текст и кнопки прямо в текущем сообщении
        await callback.message.edit_text(text, parse_mode="Markdown", reply_markup=kb)
    except Exception:
        pass  # Если текст не изменился, Telegram кидает ошибку, мы её просто игнорируем

    # Обязательно "отвечаем" на коллбэк, чтобы у кнопки исчезли часики загрузки
...
async def cmd_pay(message: Message):
    ...
    # Объединяем оба списка платежей в один
    ...
    for p in all_payments:
        # Проверяем, есть ли у платежа order_id (CryptoCloud) или id (Monobank)
        p_id = getattr(p, 'order_id', getattr(p, 'id', 'Unknown'))

        # Определяем шлюз для красоты
        gateway = "☁️ CryptoCloud" if hasattr(p, 'order_id') else "🐈 Monobank"

        # Статус есть только у CryptoCloud, для Monobank пишем "Успешно", так как мы пишем только успешные
...
async def get_gifts_page_content(page: int):
    ...
    # Переворачиваем список, чтобы новые коды были сверху
    gifts.reverse()
